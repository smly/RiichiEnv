...
mod unit_tests {
    ...
    /// Regression test for #199: replays the offending position from the
    /// riichi.dev game `16d035ad-5f2b-4bfb-b7a6-b5c728bdb4c1` (E3 0-honba,
    /// turn 13). Player 0 declared riichi after `tsumo 6m` and then discarded
    /// `6p`, leaving the hand `4m,5mr,6m,7m,8m,4p,4p,5pr,5p,6p` + ankan(7s),
    /// which is *not* tenpai. The bug fixed in #202 is that the legal-action
    /// filter previously offered every concealed tile during `riichi_stage`
    /// rather than only those that keep the hand tenpai. The only
    /// tenpai-maintaining discards from this 11-tile hand are
    /// `{4m, 5mr, 7m, 8m}` (each leaves a 3-meld + tanki shape with the
    /// pinzu folded into iipeiko `(4p5p6p)(4p5p6p)`); `6p` is not among them.
    #[test]
    fn test_issue_199_noten_riichi_discard_rejected() {
        use crate::action::{Action, ActionType};
        use crate::state::legal_actions::GameStateLegalActions;
        ...
        // 11-tile concealed hand right after `tsumo 6m`:
        // 4m=12, 5mr=16, 6m=20, 7m=24, 8m=28,
        // 4p=48,49, 5pr=52, 5p=53, 6p=56,57
        state.players[pid_us].hand = vec![12, 16, 20, 24, 28, 48, 49, 52, 53, 56, 57];
        state.players[pid_us].hand.sort();
        // Ankan 7s7s7s7s declared earlier in the kyoku (tile_ids 96..=99).
        state.players[pid_us].melds = vec![Meld {
            meld_type: MeldType::Ankan,
            tiles: vec![96, 97, 98, 99],
            opened: false,
            from_who: -1,
            called_tile: None,
        }];
        state.players[pid_us].score = 28700;
        state.players[pid_us].riichi_declared = false;
        state.players[pid_us].riichi_stage = false;
        state.players[pid_us].forbidden_discards.clear();
        state.drawn_tile = Some(20); // 6m
        state.phase = Phase::WaitAct;
        state.active_players = vec![pid];

        // Mjai-style Riichi declaration: tile = None.
        let mut actions = std::collections::HashMap::new();
        actions.insert(pid, Action::new(ActionType::Riichi, None, vec![], None));
        state.step(&actions);
        ...
        let legal = state._get_legal_actions_internal(pid);
        let discard_classes: std::collections::BTreeSet<u8> = legal
            .iter()
            .filter(|a| a.action_type == ActionType::Discard)
            .filter_map(|a| a.tile.map(|t| t / 4))
            .collect();

        // The only tenpai-maintaining discards from this hand.
        let expected: std::collections::BTreeSet<u8> =
            [3u8, 4, 6, 7].into_iter().collect(); // 4m, 5m, 7m, 8m
        assert_eq!(
            discard_classes, expected,
            "riichi_stage discards must be exactly the tenpai-maintaining set"
        );

        // The offending log discard (6p, class 14 = 56/4) must be rejected.
        assert!(
            !discard_classes.contains(&14),
            "6p must not be a legal discard during riichi_stage (was the actual \
             #199 log discard that broke tenpai)"
        );
    }
