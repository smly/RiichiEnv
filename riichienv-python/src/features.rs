...
#[pyfunction]
#[pyo3(name = "encode_sp_batch_3p")]
pub fn encode_sp_batch_3p_py<'py>(
    py: Python<'py>,
    observations: Vec<Observation3P>,
) -> PyResult<Bound<'py, PyBytes>> {
    let values = py
        .detach(move || features::encode_sp_3p_batch(&observations))
        .map_err(PyErr::from)?;
    Ok(floats_as_bytes(py, &values))
}

#[pyfunction]
#[pyo3(name = "encode_drev_batch_3p")]
pub fn encode_drev_batch_3p_py<'py>(
    py: Python<'py>,
    observations: Vec<Observation3P>,
) -> PyResult<Bound<'py, PyBytes>> {
    let values = py
        .detach(move || features::encode_drev_3p_batch(&observations))
        .map_err(PyErr::from)?;
    Ok(floats_as_bytes(py, &values))
}
