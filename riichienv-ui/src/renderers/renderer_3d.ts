...
export class Renderer3D implements IRenderer {
    ...
    render(sourceState: BoardState, debugPanel?: HTMLElement, displayOptions?: Readonly<DisplayOptions>): void {
        ...
        if (vpPlayer) {
            const handEl = this.renderOwnHand(vpPlayer, this.viewpoint, state, dangerWaitsByPlayer[this.viewpoint]);
        ...
        if (vpPlayer && (vpPlayer.melds.length > 0 || (pc === 3 && vpPlayer.kitaCount > 0))) {
            ...
            meldInner.appendChild(this.renderOwnMelds(vpPlayer, this.viewpoint));
    ...
    private renderCenter3D(state: BoardState): HTMLElement {
        ...
        state.players.forEach((p, i) => {
            ...
            const seat = relativeSeat(i, this.viewpoint);
        ...
        if (state.players[state.currentActor]) {
            ...
            const seat = relativeSeat(state.currentActor, this.viewpoint);
    ...
    private renderRiichiSticks(table: HTMLElement, state: BoardState): void {
        state.players.forEach((p, i) => {
            if (!p.riichi) return;
            const relPos = relativeSeat(i, this.viewpoint);
    ...
    private renderOpponentHandArea(
        ...
        if (player.melds.length > 0) {
            const meldsDiv = this.renderMelds(player.melds, playerIdx, activeWaits);
    ...
    // =========================================================================
    private renderOwnMelds(player: PlayerState, playerIdx: number): HTMLElement {
        const wrapper = document.createElement('div');
        wrapper.className = 'own-melds-3d';
        const melds = this.renderMelds(player.melds, playerIdx);
        melds.classList.add('own-melds-inner');
    ...
    private renderMelds(
        ...
        playerIdx: number,
        activeWaits: ReadonlySet<string> = new Set(),
    ): HTMLElement {
        const tw = this.layout.tileSizes.opponentTile[0];
        const normalize = (tile: string) => tile.replace('0', '5').replace('r', '');
        ...
        melds.forEach((m) => {
            ...
            const rel = relativeSeat(m.from, playerIdx);
            const tiles = [...m.tiles];
    ...
    private renderCallOverlay(overlay: HTMLElement, state: BoardState): void {
        ...
        if (label) {
            ...
            if (actorIdx !== undefined) {
                const relIndex = relativeSeat(actorIdx, this.viewpoint);
