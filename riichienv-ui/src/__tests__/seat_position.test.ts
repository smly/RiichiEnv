...
describe('fixed physical table edges', () => {
    it.each([0, 1, 2])('preserves players and the empty seat for every viewpoint with dealer %i', (dealer) => {
        ...
        current.players.forEach((_, viewpoint) => {
            const seats = current.players.map((_, index) => {
                const seat = relativeSeat(index, viewpoint);
    ...
    it('preserves four-player positions', () => {
        for (let viewpoint = 0; viewpoint < 4; viewpoint++) {
            for (let player = 0; player < 4; player++) {
                expect(relativeSeat(player, viewpoint)).toBe((player - viewpoint + 4) % 4);
            }
        }
    });
    it('keeps called tiles facing their physical source', () => {
        expect(relativeSeat(0, 1)).toBe(3); // Player 0 is to player 1's left.
        expect(relativeSeat(0, 2)).toBe(2); // Player 0 is opposite player 2.
        expect(relativeSeat(1, 2)).toBe(3); // Player 1 is to player 2's left.
    });
    ...
    it('updates winds without changing positions when seeking forward and backward between rounds', () => {
        ...
        for (const step of [1, 2, 3, 2, 1]) {
            ...
            for (let viewpoint = 0; viewpoint < 3; viewpoint++) {
                expect(state.players.map((_, player) => relativeSeat(player, viewpoint))).toEqual(
