...
class RiichiEnv:
    ...
    def step(self, actions: dict[int, Action]) -> dict[int, Observation]:
        """Execute one step."""
        self.agari_results = {}
        if self.is_done: return self._get_observations([])
        ...
        if self.phase == Phase.WAIT_ACT:
            ...
            if action.type == ActionType.RIICHI:
                if self.scores[self.current_player] < 1000: raise ValueError("Not enough points for Riichi")
                ...
                ura_in = self._get_ura_markers_tid() if (self.riichi_declared[self.current_player] or self.double_riichi_declared[self.current_player]) else []
                is_first = self.is_first_turn and len(self.discards[self.current_player]) == 0
                res = AgariCalculator(self.hands[self.current_player], self.melds.get(self.current_player, [])).calc(self.drawn_tile, dora_indicators=self.dora_indicators, conditions=Conditions(tsumo=True, riichi=self.riichi_declared[self.current_player], double_riichi=self.double_riichi_declared[self.current_player], ippatsu=self.ippatsu_eligible[self.current_player], rinshan=self.is_rinshan_flag, haitei=(len(self.wall) <= 14 and not self.is_rinshan_flag), player_wind=(self.current_player - self.oya + 4) % 4, round_wind=self._custom_round_wind, tsumo_first_turn=is_first), ura_indicators=ura_in)
                ...
                self.mjai_log.append({"type": "hora", "actor": self.current_player, "target": self.current_player, "tsumo": True, "pai": _to_mjai_tile(self.drawn_tile), "deltas": deltas, "ura_markers": ura_markers})
                self.mjai_log.append({"type": "end_kyoku"})
                self.mjai_log.append({"type": "end_game"})
                return self._get_observations([])
            elif action.type == ActionType.KYUSHU_KYUHAI:
                self._trigger_ryukyoku("kyushu_kyuhai"); return self._get_observations([])
                ...
                if ronners:
                    self.phase, self.active_players, self.pending_kan = Phase.WAIT_RESPONSE, ronners, (self.current_player, action)
                    ...
                    return self._get_observations(self.active_players)
                if self.drawn_tile is not None: self.hands[self.current_player].append(self.drawn_tile); self.drawn_tile = None
                self._execute_claim(self.current_player, action)
                self.ippatsu_eligible = [False] * 4; self.is_first_turn = False
                if len(self.wall) <= 14: self._trigger_ryukyoku("exhaustive_draw"); return self._get_observations([])
                self.is_rinshan_flag = True; self.drawn_tile = self.wall.pop(0)
                ...
                for pid in ronners: self.current_claims.setdefault(pid, []).append(Action(ActionType.RON, tile=discard_tile_id))
                for pid in range(4):
                    if pid == self.current_player: continue
                    for opt in self._can_pon(self.hands[pid], discard_tile_id): self.current_claims.setdefault(pid, []).append(Action(ActionType.PON, tile=discard_tile_id, consume_tiles=opt))
                    for opt in self._can_kan(self.hands[pid], discard_tile_id): self.current_claims.setdefault(pid, []).append(Action(ActionType.DAIMINKAN, tile=discard_tile_id, consume_tiles=opt))
                next_p = (self.current_player + 1) % 4
                for opt in self._can_chi(self.hands[next_p], discard_tile_id): self.current_claims.setdefault(next_p, []).append(Action(ActionType.CHI, tile=discard_tile_id, consume_tiles=opt))
            ...
            if ronners:
                ...
                for winner in sorted_ronners:
                    ...
                    if self.riichi_declared[winner] or self.double_riichi_declared[winner]:
                        ura_markers = self._get_ura_markers()
                    self.mjai_log.append({"type": "hora", "actor": winner, "target": self.current_player, "tile": _to_mjai_tile(tile), "deltas": deltas, "ura_markers": ura_markers})
                self.mjai_log.append({"type": "end_kyoku"})
                self.mjai_log.append({"type": "end_game"})
                self.is_done = True
                return self._get_observations([])
            ...
            if chiers:
                ...
                return self._get_observations(self.active_players)
            if self.pending_kan:
                claimer, act = self.pending_kan; self.pending_kan = None
                if self.drawn_tile is not None: self.hands[claimer].append(self.drawn_tile); self.drawn_tile = None
                self._execute_claim(claimer, act); self.ippatsu_eligible = [False]*4; self.is_first_turn = False
                if len(self.wall) <= 14: self._trigger_ryukyoku("exhaustive_draw"); return self._get_observations([])
                self.is_rinshan_flag, self.drawn_tile = True, self.wall.pop(0)
    ...
    def _get_observations(self, player_ids: list[int]) -> dict[int, Observation]:
        ...
        for pid in player_ids:
            hand = self.hands[pid][:]
            if pid == self.current_player and self.drawn_tile is not None: hand.append(self.drawn_tile)
            filtered_events = []
            for ev in self.mjai_log:
                ev_copy = ev.copy()
                if ev["type"] == "start_kyoku": ev_copy["tehais"] = [t if i == pid else ["?"]*len(t) for i, t in enumerate(ev["tehais"])]
                elif ev["type"] == "tsumo" and ev["actor"] != pid: ev_copy["tile"] = "?"
                filtered_events.append(ev_copy)
            obs_dict[pid] = Observation(pid, hand, filtered_events, self._player_event_counts[pid], self._get_legal_actions(pid))
    ...
    def _get_legal_actions(self, pid: int) -> list[Action]:
        ...
        if self.phase == Phase.WAIT_ACT:
            if pid != self.current_player: return []
            if self.riichi_stage[pid]:
                for t in _riichienv.check_riichi_candidates(h14): actions.append(Action(ActionType.DISCARD, tile=t))
                return actions
            ...
            if len(self.wall) > 14:
                ...
                counts = {}
                for t in h14: counts[t // 4] = counts.get(t // 4, 0) + 1
            ...
            if pid in self.current_claims: actions.extend(self.current_claims[pid])
        return actions

    def _execute_claim(self, pid: int, action: Action, from_pid: int | None = None):
        hand = self.hands[pid]
        for t in action.consume_tiles: hand.remove(t)
        m_type, opened = MeldType.Peng, True
        if action.type == ActionType.CHI: m_type = MeldType.Chi
        elif action.type == ActionType.DAIMINKAN: m_type = MeldType.Gang
        elif action.type == ActionType.ANKAN: m_type, opened = MeldType.Angang, False
        elif action.type == ActionType.KAKAN:
            m_type = MeldType.Addgang
            found = [i for i, m in enumerate(self.melds[pid]) if m.meld_type == MeldType.Peng and m.tiles[0]//4 == action.tile//4]
            if found:
                old = self.melds[pid].pop(found[0]); tiles = sorted(list(old.tiles) + [action.tile])
                self.melds[pid].append(Meld(m_type, tiles, True))
                self.mjai_log.append({"type": "kakan", "actor": pid, "tile": _to_mjai_tile(action.tile), "consumed": [_to_mjai_tile(action.tile)]})
        ...
        if from_pid is not None and m_type in [MeldType.Peng, MeldType.Gang]:
            if sum(1 for m in self.melds[pid] if m.tiles[0]//4 in [31,32,33]) == 3: self.pao[pid][37] = from_pid
            if sum(1 for m in self.melds[pid] if m.tiles[0]//4 in [27,28,29,30]) == 4: self.pao[pid][50] = from_pid
        mj_types = {ActionType.CHI: "chi", ActionType.PON: "pon", ActionType.DAIMINKAN: "daiminkan", ActionType.ANKAN: "ankan"}
        self.mjai_log.append({"type": mj_types[action.type], "actor": pid, "target": from_pid if from_pid is not None else -1, "tile": _to_mjai_tile(action.tile), "consumed": [_to_mjai_tile(t) for t in action.consume_tiles]})
        if action.type == ActionType.ANKAN: self._reveal_kan_dora(False)
        elif action.type == ActionType.DAIMINKAN: self.pending_kan_dora_count += 1

    def _can_pon(self, hand: list[int], tile: int) -> list[list[int]]:
        matches = [t for t in hand if t // 4 == tile // 4]
        if len(matches) < 2: return []
        reds = [t for t in matches if t in [16, 52, 88]]; norms = [t for t in matches if t not in [16, 52, 88]]
        opts = []
        if len(norms) >= 2: opts.append([norms[0], norms[1]])
        if len(norms) >= 1 and len(reds) >= 1: opts.append(sorted([norms[0], reds[0]]))
        return opts
    ...
    def _can_chi(self, hand: list[int], tile: int) -> list[list[int]]:
        ...
        def get_cand(r):
            ...
            return res
        pairs = []
        if idx >= 2: pairs.append((t_type-2, t_type-1))
        if 1 <= idx <= 7: pairs.append((t_type-1, t_type+1))
        if idx <= 6: pairs.append((t_type+1, t_type+2))
        for r1, r2 in pairs:
    ...
    def _reveal_kan_dora(self, post_rinshan: bool = True):
        count = len(self.dora_indicators); idx = 5 + count - self.pending_kan_dora_count
        if count >= 5: return
        if 0 <= idx < len(self.wall):
            t = self.wall[idx]; self.dora_indicators.append(t); self.mjai_log.append({"type": "dora", "dora_marker": _to_mjai_tile(t)})
    ...
    def _get_ura_markers_tid(self) -> list[int]:
        ...
        for d in self.dora_indicators:
            try:
                i = self.wall.index(d)
                if i+1 < len(self.wall): res.append(self.wall[i+1])
            except: pass
        return res
    ...
    def _get_ron_potential(self, tile: int, is_chankan: bool, is_ankan: bool = False) -> list[int]:
        ...
        for p in range(4):
            ...
            calc = AgariCalculator(self.hands[p], self.melds.get(p, [])).calc(tile, dora_indicators=self.dora_indicators, conditions=Conditions(tsumo=False, riichi=self.riichi_declared[p], double_riichi=self.double_riichi_declared[p], ippatsu=self.ippatsu_eligible[p], player_wind=(p - self.oya + 4)%4, round_wind=self._custom_round_wind, houtei=(len(self.wall) <= 14), chankan=is_chankan))
            if calc.agari:
                if is_ankan and not (calc.han >= 13 and any(y in [42, 49] for y in calc.yaku)): continue
                res.append(p)
        return res
    ...
    def _check_midway_draws(self) -> bool:
        discards = []
        for p in range(4):
            if len(self.discards[p]) != 1: break
            discards.append(self.discards[p][0])
        else:
            if len(discards) == 4:
                t = discards[0] // 4
                if 27 <= t <= 30 and all(d // 4 == t for d in discards) and not any(len(m) for m in self.melds.values()): self._trigger_ryukyoku("sufuurenta"); return True
        tk, kp = 0, set()
