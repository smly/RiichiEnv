"use strict";var RiichiEnvViewer=(()=>{var b=Object.defineProperty;var z=Object.getOwnPropertyDescriptor;var P=Object.getOwnPropertyNames;var _=Object.prototype.hasOwnProperty;var U=(l,n)=>{for(var e in n)b(l,e,{get:n[e],enumerable:!0})},S=(l,n,e,t)=>{if(n&&typeof n=="object"||typeof n=="function")for(let o of P(n))!_.call(l,o)&&o!==e&&b(l,o,{get:()=>n[o],enumerable:!(t=z(n,o))||t.enumerable});return l};var O=l=>S(b({},"__esModule",{value:!0}),l);var F={};U(F,{Viewer:()=>g});var D=l=>{let n=e=>{if(e==="back")return 9999;let t=e.slice(-1),o=parseInt(e[0]);return e.endsWith("r")&&(t=e.slice(-2,-1),o=5),({m:0,p:10,s:20,z:30}[t]||0)*10+o+(e.endsWith("r")?.5:0)};return[...l].sort((e,t)=>n(e)-n(t))},w=class{constructor(n){this.events=n,this.cursor=0,this.current=this.initialState()}getKyokuCheckpoints(){let n=[];return this.events.forEach((e,t)=>{e.type==="start_kyoku"&&n.push({index:t,round:(e.kyoku||1)-1,honba:e.honba||0})}),n}initialState(){return{players:Array(4).fill(0).map(()=>({hand:[],discards:[],melds:[],score:25e3,riichi:!1,wind:0})),doraMarkers:[],round:0,honba:0,kyotaku:0,wallRemaining:70,currentActor:0}}stepForward(){if(this.cursor>=this.events.length)return!1;let n=this.events[this.cursor];return this.processEvent(n),this.cursor++,!0}stepBackward(){if(this.cursor<=0)return!1;let n=this.cursor-1;for(this.reset();this.cursor<n;)this.stepForward();return!0}jumpTo(n){for(n<0&&(n=0),n>this.events.length&&(n=this.events.length),n<this.cursor&&this.reset();this.cursor<n;)this.stepForward()}reset(){this.cursor=0,this.current=this.initialState()}processEvent(n){switch(n.type){case"start_game":break;case"start_kyoku":this.current.round=(n.kyoku||1)-1,this.current.honba=n.honba||0,this.current.kyotaku=n.kyotaku||0,this.current.doraMarkers=[n.dora_marker],this.current.currentActor=n.oya,this.current.players.forEach((e,t)=>{e.hand=n.tehais[t].map(o=>o),e.discards=[],e.melds=[],e.riichi=!1,e.score=n.scores[t]});break;case"tsumo":n.actor!==void 0&&n.pai&&(this.current.players[n.actor].hand.push(n.pai),this.current.currentActor=n.actor);break;case"dahai":if(n.actor!==void 0&&n.pai){let e=this.current.players[n.actor],t=e.hand.indexOf(n.pai);t>=0&&e.hand.splice(t,1),e.hand=D(e.hand),e.discards.push(n.pai),this.current.currentActor=n.actor}break;case"pon":case"chi":case"daiminkan":if(n.actor!==void 0&&n.target!==void 0&&n.pai&&n.consumed){let e=this.current.players[n.actor];n.consumed.forEach(o=>{let p=e.hand.indexOf(o);p>=0&&e.hand.splice(p,1)}),e.melds.push({type:n.type,tiles:[...n.consumed,n.pai],from:n.target}),this.current.currentActor=n.actor;let t=this.current.players[n.target];t.discards.length>0&&t.discards.pop()}break;case"kakan":case"ankan":if(n.actor!==void 0&&n.consumed){let e=this.current.players[n.actor];n.consumed.forEach(t=>{let o=e.hand.indexOf(t);o>=0&&e.hand.splice(o,1)}),e.melds.push({type:n.type,tiles:n.consumed,from:n.actor})}break;case"reach":n.actor!==void 0&&n.step==="2"&&(this.current.players[n.actor].riichi=!0,this.current.kyotaku+=1,this.current.players[n.actor].score-=1e3);break;case"dora":n.dora_marker&&this.current.doraMarkers.push(n.dora_marker);break;case"hora":case"ryukyoku":n.scores&&this.current.players.forEach((e,t)=>e.score=n.scores[t]);break}}};var f={back:`<?xml version="1.0" encoding="UTF-8" standalone="no"?>
...
`};var x=class{constructor(n){this.container=n;let e=document.getElementById("riichienv-viewer-style");e||(e=document.createElement("style"),e.id="riichienv-viewer-style",e.textContent=`
                ...
                .tile-fg { z-index: 2; }
            `,document.head.appendChild(e)),this.styleElement=e}getTileHtml(n){if(n==="back")return`<div class="tile-layer"><div class="tile-bg">${f.back||f.blank}</div></div>`;let e=f.front||"",t=f[n];return t||(console.warn(`Missing SVG for tile: ${n}`),t=f.blank||""),`
            <div class="tile-layer">
                <div class="tile-bg">${e}</div>
                <div class="tile-fg">${t}</div>
            </div>
        `}render(n){Array.from(this.container.children).forEach(s=>{s.tagName!=="STYLE"&&this.container.removeChild(s)}),this.styleElement&&!this.container.contains(this.styleElement)&&this.container.appendChild(this.styleElement);let t=document.createElement("div");t.className="mahjong-board",Object.assign(t.style,{position:"relative",width:"100%",aspectRatio:"1/1",maxWidth:"800px",margin:"0 auto",backgroundColor:"#2d5a27",borderRadius:"8px",overflow:"hidden",fontSize:"12px",color:"white",fontFamily:"sans-serif",boxSizing:"border-box"});let o=document.createElement("div");o.className="center-info",Object.assign(o.style,{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%, -50%)",backgroundColor:"#1a3317",padding:"10px",borderRadius:"5px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:"10"}),o.innerHTML=`
            <div>${this.formatRound(n.round)} - Honba: ${n.honba}</div>
            <div>Kyotaku: ${n.kyotaku}</div>
            <div style="margin-top:5px; display:flex; gap:2px;">
                Dora: ${n.doraMarkers.map(s=>`<div style="width:24px; height:34px;">${this.getTileHtml(s)}</div>`).join("")}
            </div>
        `,t.appendChild(o);let p=[0,-90,180,90];n.players.forEach((s,h)=>{let y=document.createElement("div");Object.assign(y.style,{position:"absolute",top:"50%",left:"50%",width:"0",height:"0",display:"flex",justifyContent:"center",transform:`rotate(${p[h]}deg)`});let r=document.createElement("div");Object.assign(r.style,{width:"600px",height:"auto",minHeight:"200px",display:"flex",flexDirection:"column",alignItems:"center",transform:"translateY(120px)"});let i=document.createElement("div");i.textContent=`P${h}: ${s.score}`,s.riichi&&(i.style.color="#ff6b6b"),i.style.fontWeight="bold",i.style.marginBottom="5px",i.style.textShadow="1px 1px 2px black";let a=document.createElement("div");Object.assign(a.style,{display:"flex",flexWrap:"wrap",width:"150px",minHeight:"80px",marginBottom:"10px",justifyContent:"center"}),s.discards.forEach(k=>{let d=document.createElement("div");d.style.width="25px",d.style.height="35px",d.innerHTML=this.getTileHtml(k),a.appendChild(d)});let c=document.createElement("div");c.style.display="flex",c.style.height="42px",c.style.justifyContent="center";let v=document.createElement("div");if(v.style.display="flex",s.hand.forEach(k=>{let d=document.createElement("div");d.style.width="30px",d.style.height="42px",d.innerHTML=this.getTileHtml(k),v.appendChild(d)}),c.appendChild(v),s.melds.length>0){let k=document.createElement("div");k.style.display="flex",k.style.marginLeft="10px",s.melds.forEach(d=>{let u=document.createElement("div");u.style.display="flex",u.style.gap="0",d.tiles.forEach(j=>{let m=document.createElement("div");m.style.width="30px",m.style.height="42px",m.innerHTML=this.getTileHtml(j),u.appendChild(m)}),k.appendChild(u)}),c.appendChild(k)}r.appendChild(i),r.appendChild(a),r.appendChild(c),y.appendChild(r),t.appendChild(y)}),this.container.appendChild(t)}formatRound(n){return`${["East","South","West","North"][Math.floor(n/4)]} ${n%4+1}`}};var g=class{constructor(n,e){let t=document.getElementById(n);if(!t)throw new Error(`Container #${n} not found`);this.container=t,this.log=e,this.container.innerHTML="";let o=document.createElement("div");o.id=`${n}-board`,Object.assign(o.style,{display:"flex",justifyContent:"center",alignItems:"flex-start",width:"100%",overflow:"auto"}),this.container.appendChild(o),this.controlPanel=document.createElement("div"),Object.assign(this.controlPanel.style,{display:"flex",gap:"10px",justifyContent:"center",marginTop:"10px",padding:"10px",backgroundColor:"#f0f0f0",borderRadius:"4px"}),this.container.appendChild(this.controlPanel),this.gameState=new w(e),this.renderer=new x(o),this.initControls(),this.update()}initControls(){let n=document.createElement("select");n.style.padding="5px",n.style.marginRight="10px";let e=this.gameState.getKyokuCheckpoints(),t=document.createElement("option");t.value="0",t.text="Start",n.appendChild(t),e.forEach(i=>{let a=document.createElement("option");a.value=i.index.toString(),a.text=`${this.renderer.formatRound(i.round)} - ${i.honba} Honba`,n.appendChild(a)}),n.addEventListener("change",()=>{let i=parseInt(n.value);this.gameState.jumpTo(i),this.update()}),this.kyokuSelect=n,this.controlPanel.appendChild(n);let o="padding: 5px 10px; cursor: pointer;",p=(i,a)=>{let c=document.createElement("button");return c.textContent=i,c.style.cssText=o,c.onclick=a,c},s=p("< Prev",()=>{this.gameState.stepBackward()&&this.update()}),h=p("Next >",()=>{this.gameState.stepForward()&&this.update()}),y=p("<< Start",()=>{this.gameState.reset(),this.update()}),r=document.createElement("input");r.type="range",r.min="0",r.max=String(this.log.length),r.value="0",r.style.flexGrow="1",r.oninput=i=>{let a=parseInt(i.target.value);this.gameState.jumpTo(a),this.update()},this.slider=r,this.controlPanel.appendChild(y),this.controlPanel.appendChild(s),this.controlPanel.appendChild(r),this.controlPanel.appendChild(h)}update(){this.renderer.render(this.gameState.current),this.slider.value=String(this.gameState.cursor);let n=this.gameState.getKyokuCheckpoints(),e=0;for(let t=0;t<n.length&&n[t].index<=this.gameState.cursor;t++)e=t;n.length>0&&(this.kyokuSelect.value=n[e].index.toString())}};window.RiichiEnvViewer=g;return O(F);})();
