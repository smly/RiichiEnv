...
`};var w=class{constructor(n){if(this.container=n,!document.getElementById("riichienv-viewer-style")){let e=document.createElement("style");e.id="riichienv-viewer-style",e.textContent=`
                ...
                .tile-fg { z-index: 2; }
            `,this.container.appendChild(e),this.styleElement=e}}getTileHtml(n){if(n==="back")return`<div class="tile-layer"><div class="tile-bg">${y.back||y.blank}</div></div>`;let e=y.front||"",t=y[n];return t||(console.warn(`Missing SVG for tile: ${n}`),t=y.blank||""),`
            <div class="tile-layer">
                <div class="tile-bg">${e}</div>
                <div class="tile-fg">${t}</div>
            </div>
        `}render(n){Array.from(this.container.children).forEach(i=>{i.tagName!=="STYLE"&&this.container.removeChild(i)}),this.styleElement&&!this.container.contains(this.styleElement)&&this.container.appendChild(this.styleElement);let t=document.createElement("div");t.className="mahjong-board",Object.assign(t.style,{position:"relative",width:"100%",aspectRatio:"1/1",maxWidth:"800px",margin:"0 auto",backgroundColor:"#2d5a27",borderRadius:"8px",overflow:"hidden",fontSize:"12px",color:"white",fontFamily:"sans-serif",boxSizing:"border-box"});let o=document.createElement("div");o.className="center-info",Object.assign(o.style,{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%, -50%)",backgroundColor:"#1a3317",padding:"10px",borderRadius:"5px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:"10"}),o.innerHTML=`
            <div>${this.formatRound(n.round)} - Honba: ${n.honba}</div>
            <div>Kyotaku: ${n.kyotaku}</div>
            <div style="margin-top:5px; display:flex; gap:2px;">
                Dora: ${n.doraMarkers.map(i=>`<div style="width:24px; height:34px;">${this.getTileHtml(i)}</div>`).join("")}
            </div>
        `,t.appendChild(o);let c=[0,-90,180,90];n.players.forEach((i,p)=>{let d=document.createElement("div");Object.assign(d.style,{position:"absolute",top:"50%",left:"50%",width:"0",height:"0",display:"flex",justifyContent:"center",transform:`rotate(${c[p]}deg)`});let l=document.createElement("div");Object.assign(l.style,{width:"600px",height:"auto",minHeight:"200px",display:"flex",flexDirection:"column",alignItems:"center",transform:"translateY(120px)"});let k=document.createElement("div");k.textContent=`P${p}: ${i.score}`,i.riichi&&(k.style.color="#ff6b6b"),k.style.fontWeight="bold",k.style.marginBottom="5px",k.style.textShadow="1px 1px 2px black";let g=document.createElement("div");Object.assign(g.style,{display:"flex",flexWrap:"wrap",width:"150px",minHeight:"80px",marginBottom:"10px",justifyContent:"center"}),i.discards.forEach(a=>{let s=document.createElement("div");s.style.width="25px",s.style.height="35px",s.innerHTML=this.getTileHtml(a),g.appendChild(s)});let f=document.createElement("div");f.style.display="flex",f.style.height="42px",f.style.justifyContent="center";let v=document.createElement("div");if(v.style.display="flex",i.hand.forEach(a=>{let s=document.createElement("div");s.style.width="30px",s.style.height="42px",s.innerHTML=this.getTileHtml(a),v.appendChild(s)}),f.appendChild(v),i.melds.length>0){let a=document.createElement("div");a.style.display="flex",a.style.marginLeft="10px",i.melds.forEach(s=>{let h=document.createElement("div");h.style.display="flex",h.style.gap="0",s.tiles.forEach(b=>{let u=document.createElement("div");u.style.width="30px",u.style.height="42px",u.innerHTML=this.getTileHtml(b),h.appendChild(u)}),a.appendChild(h)}),f.appendChild(a)}l.appendChild(k),l.appendChild(g),l.appendChild(f),d.appendChild(l),t.appendChild(d)}),this.container.appendChild(t)}formatRound(n){return`${["East","South","West","North"][Math.floor(n/4)]} ${n%4+1}`}};var x=class{constructor(n,e){let t=document.getElementById(n);if(!t)throw new Error(`Container #${n} not found`);this.container=t,this.log=e,this.container.innerHTML="";let o=document.createElement("div");o.id=`${n}-board`,Object.assign(o.style,{display:"flex",justifyContent:"center",alignItems:"flex-start",width:"100%",overflow:"auto"}),this.container.appendChild(o),this.controlPanel=document.createElement("div"),Object.assign(this.controlPanel.style,{display:"flex",gap:"10px",justifyContent:"center",marginTop:"10px",padding:"10px",backgroundColor:"#f0f0f0",borderRadius:"4px"}),this.container.appendChild(this.controlPanel),this.gameState=new m(e),this.renderer=new w(o),this.initControls(),this.update()}initControls(){let n="padding: 5px 10px; cursor: pointer;",e=(p,d)=>{let l=document.createElement("button");return l.textContent=p,l.style.cssText=n,l.onclick=d,l},t=e("< Prev",()=>{this.gameState.stepBackward()&&this.update()}),o=e("Next >",()=>{this.gameState.stepForward()&&this.update()}),c=e("<< Start",()=>{this.gameState.reset(),this.update()}),i=document.createElement("input");i.type="range",i.min="0",i.max=String(this.log.length),i.value="0",i.style.flexGrow="1",i.oninput=p=>{let d=parseInt(p.target.value);this.gameState.jumpTo(d),this.update()},this.slider=i,this.controlPanel.appendChild(c),this.controlPanel.appendChild(t),this.controlPanel.appendChild(i),this.controlPanel.appendChild(o)}update(){this.renderer.render(this.gameState.current),this.slider.value=String(this.gameState.cursor)}};window.RiichiEnvViewer=x;return O(F);})();
